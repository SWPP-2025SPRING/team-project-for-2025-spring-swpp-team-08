...
def save_obj(vertices, faces, path):
    ...
    lines += [
        f"f {' '.join(str(i+1) for i in face)}" for face in faces
    ]  # 0-인덱싱을 1-인덱싱으로 변환
...
def extrude_thickness_from_top(
    vertices, faces, edge_parts, thickness, edge_resolution, reverse_winding=False
):
    if thickness <= 0:
    ...
    bottom_vertices_list = [
        (vx, vy_orig - thickness, vz) for (vx, vy_orig, vz) in vertices
    ]
    ...
    if loop_len == 0 or edge_resolution_scaled == 0:
        if not full_loop:
            print(
                f"경고: 압출을 위한 전체 루프가 비어 있습니다. 옆면/바닥면이 생성되지 않았습니다."
            )
        return vertices, faces
    ...
    if not sampled_indices or len(sampled_indices) < 2:  # 유효한 루프인지 확인
        print(
            f"경고: 압출을 위한 샘플링된 루프가 너무 짧습니다. 옆면/바닥면이 생성되지 않았습니다."
        )
        return vertices, faces
    ...
    if len(bottom_loop_verts_indices) >= 3:  # 폴리곤은 최소 3개의 정점 필요
        ...
        print(
            f"경고: 밑면을 만들기에 정점이 충분하지 않습니다 ({len(bottom_loop_verts_indices)}개 발견)."
        )
...
def get_square_edge_loop(resolution):
    ...
    right = [
        i * resolution + (resolution - 1) for i in range(1, resolution)
    ]  # 시작점 중복 피하기 위해 1부터
    top = [
        resolution * (resolution - 1) + i for i in range(resolution - 2, -1, -1)
    ]  # 끝점, 시작점 중복 피하기
    left = [
        (i * resolution) for i in range(resolution - 2, 0, -1)
    ]  # 끝점, 시작점 중복 피하기
...
def generate_cant(
    ...
    z_coords = np.linspace(
        -tile_size / 2, tile_size / 2, resolution
    )  # 도로는 XZ 평면에 놓임
    ...
    vertices, faces = extrude_thickness_from_top(
        vertices, faces, edge_parts, thickness, edge_resolution, reverse_winding=False
    )
...
def generate_s_curve(
    ...
    # 윗면이 CCW이므로 reverse_winding=False 전달
    vertices, faces = extrude_thickness_from_top(
        vertices, faces, edge, thickness, edge_resolution, reverse_winding=False
    )
    save_obj(vertices, faces, os.path.join(output_dir, f"{name}.obj"))
...
def generate_curve(
    ...
    rv, thetav = np.meshgrid(
        r_coords, theta_coords, indexing="ij"
    )  # ij 인덱싱: rv가 행(r), thetav가 열(theta)

    x = rv * np.cos(thetav)
    z = rv * np.sin(thetav)

    falloff_r = np.clip((rv - radius_inner) / edge_margin, 0, 1) * np.clip(
        (radius_outer - rv) / edge_margin, 0, 1
    )
    falloff_theta = np.ones_like(thetav)
    if angle_rad > 1e-6:  # 유효한 각도일 때만 폴오프 적용
        falloff_theta = np.clip(thetav / safe_theta_margin_num, 0, 1) * np.clip(
            (angle_rad - thetav) / safe_theta_margin_den_end, 0, 1
        )

    bump_noise = generate_noise_bump(
        (res_r, res_theta), sigma, bump_strength, falloff_r * falloff_theta
    )
    y_center_offset = thickness / 2
    ...
    part1 = [
        i * res_theta + 0 for i in range(res_r)
    ]  # theta = 0 (시작 각도), r_inner 부터 r_outer 까지
    part2 = []
    if res_theta > 1:
        part2 = [
            (res_r - 1) * res_theta + j for j in range(1, res_theta)
        ]  # r = r_outer, theta 증가 (첫 정점 제외)
    part3 = []
    if res_r > 1 and res_theta > 1:
        part3 = [
            i * res_theta + (res_theta - 1) for i in range(res_r - 2, -1, -1)
        ]  # theta = max_angle, r_outer 부터 r_inner 까지 (양 끝 정점 제외)
    part4 = []
    if res_r > 1 and res_theta > 2:
        part4 = [
            0 * res_theta + j for j in range(res_theta - 2, 0, -1)
        ]  # r = r_inner, theta 감소 (양 끝 정점 제외)

    effective_edge_parts_curve = {
        "bottom": part1,
        "right": part2,
        "top": part3,
        "left": part4,
...
def generate_spiral(
    ...
    theta_coords_ref = np.linspace(
        0, angle_rad, res_theta
    )  # 높이 정규화 및 범프 폴오프용 (0 ~ angle_rad)

    current_theta_coords = np.linspace(
        0, angle_rad, res_theta
    )  # 실제 정점 생성용 theta 배열
    if reverse:
    ...
    normalized_theta_for_height = (thetav - current_theta_coords[0]) / (
        current_theta_coords[-1] - current_theta_coords[0] + 1e-9
    )  # 분모 0 방지
    ...
    if angle_rad > 1e-6:  # 유효한 각도일 때만 폴오프 적용
        falloff_theta = np.clip(
            falloff_thetav_ref / theta_falloff_margin_num, 0, 1
        ) * np.clip(
            (angle_rad - falloff_thetav_ref) / theta_falloff_margin_den_end, 0, 1
        )

    bump = generate_noise_bump(
        (res_r, res_theta), sigma, bump_strength, falloff_r * falloff_theta
    )
    ...
    for i in range(res_r):
        for j in range(res_theta):
            vertices.append(
                (x[i, j], y_top_surface_values[i, j] + y_center_offset, z[i, j])
            )
    ...
    if thickness > 0:
        ...
        for i in range(res_r):
            for j in range(res_theta):
                ...
                bottom_vertices_list.append(
                    (vx_top, vy_top_with_offset - thickness, vz_top)
                )
        ...
        for i in range(res_r - 1):
            for j in range(res_theta - 1):
                ...
                if reverse:
                    ...
                    faces.append(
                        (v00_bot, v10_bot, v11_bot, v01_bot)
                    )  # 사용자가 수정한 밑면 CCW 와인딩
...
def generate_ramp_curve(
    ...
    part2_rc = []
    if res_theta > 1:
        part2_rc = [(res_r - 1) * res_theta + j for j in range(1, res_theta)]
    part3_rc = []
    if res_r > 1 and res_theta > 1:
        part3_rc = [i * res_theta + (res_theta - 1) for i in range(res_r - 2, -1, -1)]
    part4_rc = []
    if res_r > 1 and res_theta > 2:
        part4_rc = [0 * res_theta + j for j in range(res_theta - 2, 0, -1)]

    effective_edge_parts_ramp_curve = {
        "bottom": part1_rc,
        "right": part2_rc,
        "top": part3_rc,
        "left": part4_rc,
...
def main():
    output_dir = f"{os.path.dirname(os.path.abspath(__file__))}/export2"

    # Cant roads (scaled and softened)
    generate_cant(
        "CantMediumLeft",
        output_dir,
        curve_amount=-0.5 * 6,
    ...
    # Straight road (longer and wider)
    generate_straight(
        "Straight",
        output_dir,
        tile_length=4.0 * 6,
    ...
    # S-Curve roads (longer, wider, gentler)
    generate_s_curve(
        "SCurveLeft",
        output_dir,
        flip=False,
    ...
    # Circular Curves (larger radius = gentler turn)
    generate_curve(
        "CurveLargeLeft",
        output_dir,
        radius_outer=8.0 * 6,
    ...
    # Spiral (gentle spiral, more room)
    generate_spiral(
        "Spiral",
        output_dir,
        reverse=False,
    ...
    # Ramp curves (wider, gentler height change and curve)
    generate_ramp_curve(
        "RampCurveLeft",
        output_dir,
        curve_dir="left",
    ...
    # Bumped versions (same scale but with surface detail)
    generate_cant(
        "BumpedCantMediumLeft",
        output_dir,
        curve_amount=-0.5 * 6,
        tile_size=4.0 * 6,
        resolution=60,
